...
|-----------|-------------------|----------------------|
| **Separation of concerns** | Translators work in TMS; engineering owns source structure | JSON source → `l10n4x build` → signed `.lpk`; runtime never parses JSON |
...
| **Namespace ownership** | One bounded context per team or domain | Modular bundles: `{locale}/{namespace}.lpk` + `namespaces.json` manifest |
...
| **Localization** | Translated JSON per locale/namespace | Runtime code, `.lpk` signing |
...
locales/
  en/
    common.json      →  dist/en/common.lpk
    auth.json        →  dist/en/auth.lpk
    billing.json     →  dist/en/billing.lpk
  es/
    common.json
    auth.json
    billing.json
...
2. **`l10n4x build`** — emit signed `.lpk` files and language bindings.
3. **Artifact attestation** — store lpk SHA-256 + signature metadata alongside release notes (supply-chain audit).

### Secrets (never in client repos)

| Secret | Where | Purpose |
|--------|-------|---------|
| `L10N4X_SIGNING_KEY` | CI secret manager only | Ed25519 seed — signs all `.lpk` files |
...
hotfix      →  OTA reload signed lpk           →  rollback if metrics spike
```
...
| **Web (SSR)** | `@l10n4x/runtime` with `fsLpkLoader` | Node reads `.lpk` from disk at request time |
| **Native services** | `l10n4c` FFI | `l10n4c_load_lpk`, `l10n4c_load_namespace`, thread-safe RCU reads |
...
1. Download signed `.lpk` from a trusted endpoint.
2. Verify Ed25519 signature (automatic in `try_ota_reload_lpk`).
3. Atomic `swap_store` — readers never block.
4. Retain one retired snapshot per locale for `try_ota_rollback`.

Monitor:

- `lpk_reload_total` — successful hot swaps
- `lpk_verify_failures` — rejected tampered or incompatible artifacts
- `lpk_rollback_total` — manual or automated rollbacks
...
Scoped OTA (`l10n4c_store_ota_reload_lpk` / `try_ota_reload_lpk_for_store`) isolates hot reloads per tenant store — one tenant's webhook cannot swap another tenant's lpks.

---

## Multi-tenant translations (Go / Rust server)

Use **one scoped store per tenant** for lpk isolation; pass **locale per request** from session/JWT (unchanged `translate(locale, …)` signature).

### Rust (`l10n4x-core`)

1. **On tenant provision:** `create_store()` → load tenant lpks via `try_load_lpk_locale_for_store` / `try_load_static_bytes_for_store`
2. **Per HTTP request:** `translate_for_store(Some(handle), user.locale, key_hash, …)`
3. **OTA webhook (tenant-scoped):** `try_ota_reload_lpk_for_store(Some(handle), locale, bytes)`
...
1. **On tenant provision:** `h := l10n4c_store_create()` → `l10n4c_store_load_lpk_locale(h, locale, path)`
2. **Per HTTP request:** `l10n4c_store_translate(h, user.Locale, key_hash, buf, max_len)` — locale from session/JWT
3. **OTA webhook:** `l10n4c_store_ota_reload_lpk(h, locale, bytes, len)`
...
| `lpk_verify_failures` | OTA / load path | Block CDN node, rotate keys if needed |
...
- **Integrity**: mandatory Ed25519 on every `.lpk` (tamper detection in transit and at rest).
...
|-------|-------|---------------|
| **1 — Pilot** | Single locale, monolith `.lpk`, one binding target | `validate` + `build` in CI; smoke tests green |
| **2 — Modular** | Namespaces per team; fallback chain | `namespaces.json` manifest; per-team ownership in CODEOWNERS |
| **3 — OTA** | Hot reload for web/native services | Rollback tested; `lpk_*` metrics in dashboard |
| **4 — TMS** (P2.1) | Crowdin / Lokalise / Phrase sync | Translators never edit `.lpk` directly |
...
| Unsigned `.lpk` in production | Supply-chain gap — equivalent to skipping code signing |
...
Use `l10n4x sync` to exchange locale JSON with translation teams and push signed lpks to a webhook. See [TMS.md](./TMS.md).

## Related documents

- [TMS.md](./TMS.md) — export/import and webhook push
- [ARCHITECTURE.md](./ARCHITECTURE.md) — package layout and data flow
- [LPK_FORMAT.md](./LPK_FORMAT.md) — binary format and versioning

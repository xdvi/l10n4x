...
use l10n4x_core::loader::{load_raw_bytes, try_load_lpk_bytes};
...
fn ota_reload_and_rollback_roundtrip() {
    ...
    let lpk_v2 = sign_l10n(&make_l10n("greeting", "hello-v2"));
    try_ota_reload_lpk("en", &lpk_v2).expect("ota reload");
...
fn ota_verify_failure_increments_metric() {
    ...
    assert!(try_ota_reload_lpk("en", b"not-a-lpk").is_err());
    let after = metrics::metrics_string();
    assert_ne!(before, after);
}

#[test]
fn ota_reload_via_try_load_lpk_bytes_baseline() {
    let _lock = OTA_TEST_MUTEX.lock().unwrap_or_else(|e| e.into_inner());
    install_test_keys();
    clear_translations();
    let lpk = sign_l10n(&make_l10n("k", "v"));
    try_load_lpk_bytes("en", &lpk).expect("load signed lpk");
}

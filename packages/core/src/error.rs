...
pub enum CoreError {
    ...
    /// Store handle is zero or not registered.
    InvalidStoreHandle(u32),
}
...
impl core::fmt::Display for CoreError {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            ...
            CoreError::InvalidStoreHandle(id) => write!(f, "invalid store handle {id}"),
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn display_invalid_store_handle() {
        let e = CoreError::InvalidStoreHandle(42);
        assert_eq!(format!("{}", e), "invalid store handle 42");
    }

...
/// Current L10N format version (20-byte header + `min_runtime_version`).
...
impl<'a> BinaryFormatReader<'a> {
    ...
    pub fn new(data: &'a [u8]) -> CoreResult<Self> {
        ...
        let version = read_u32_be(data, 4).ok_or(crate::CoreError::InvalidFormat(
            "truncated version field",
        ))?;
        let (header_size, min_runtime_version) = match version {
            FORMAT_VERSION_V1 => (HEADER_SIZE_V1, 0),
            FORMAT_VERSION_V2 => {
    ...
    fn index_offset(&self) -> usize {
        let off = if self.version >= FORMAT_VERSION_V2 {
            12
        } else {
            8
        };
...
/// Packs sorted value blobs into an L10N v2 buffer.
#[cfg(feature = "std")]
pub fn pack_l10n(
    entries: &BTreeMap<u64, Vec<u8>>,
    min_runtime_version: u32,
    #[cfg(feature = "debug-keys")] debug_keys: Option<&BTreeMap<u64, String>>,
    #[cfg(not(feature = "debug-keys"))] _debug_keys: Option<&BTreeMap<u64, String>>,
) -> Vec<u8> {
    let mut data_pool = Vec::new();
    let mut index_entries = Vec::with_capacity(entries.len());
    let mut current_offset = HEADER_SIZE_V2 as u32;

    for (&hash, val_bytes) in entries {
    ...
    let mut buffer = Vec::with_capacity(HEADER_SIZE_V2 + data_pool.len());
    buffer.extend_from_slice(b"L10N");
    buffer.extend_from_slice(&FORMAT_VERSION_V2.to_be_bytes());
...
mod tests {
    ...
    fn create_minimal_binary() -> Vec<u8> {
        pack_l10n(&BTreeMap::new(), RUNTIME_VERSION, None)
    }
    ...
    fn new_rejects_bad_magic() {
        let data = b"XXXX\x00\x00\x00\x02\x00\x00\x00\x01\x00\x00\x00\x14\x00\x00\x00\x00\x00\x00\x00";
    ...
    fn lookup_roundtrip_v2() {
        ...
        let buf = pack_l10n(&entries, RUNTIME_VERSION, None);
    ...
    fn merge_l10n_overrides_on_collision() {
        ...
        let merged = merge_l10n_buffers(
            &pack_l10n(&left, RUNTIME_VERSION, None),
            &pack_l10n(&right, RUNTIME_VERSION, None),
        )
        .unwrap();
    ...
    fn new_rejects_future_min_runtime() {
        let buf = pack_l10n(&BTreeMap::new(), RUNTIME_VERSION + 1, None);
    ...
    fn debug_key_table_roundtrip() {
        let mut entries = BTreeMap::new();
        entries.insert(hash("common.welcome"), b"hi".to_vec());
        let mut keys = BTreeMap::new();
        keys.insert(hash("common.welcome"), "common.welcome".to_string());
        let buf = pack_l10n(&entries, RUNTIME_VERSION, Some(&keys));
        let reader = BinaryFormatReader::new(&buf).unwrap();

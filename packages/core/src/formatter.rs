...
use crate::locale_data::TIMEZONE_PARAM;
use crate::mf2_functions::{
    format_test_function, parse_options, resolve_selector_states,
    resolve_test_function_from_operand, select_mf2_variant, DeclExpr, SelectorState,
    TestFunctionValue,
};
...
#[inline]
fn write_localized<W: core::fmt::Write>(
    locale: &str,
    writer: &mut W,
    text: &str,
) -> core::fmt::Result {
...
struct PluralCaseExtras {
    exact_val: Option<f64>,
    range_min: Option<i32>,
    range_max: Option<i32>,
}

#[inline]
fn count_in_range(count_i: i64, min: i32, max: i32) -> bool {
    count_i >= i64::from(min) && count_i <= i64::from(max)
}

#[inline]
fn plural_case_matches(
    case_type: u8,
    extras: &PluralCaseExtras,
    count_f: f64,
    count_i: i64,
    cat: PluralCategory,
) -> bool {
    match case_type {
        0x01 => (count_f - extras.exact_val.unwrap_or(0.0)).abs() < 1e-9,
        ...
        0x07 => count_in_range(
            count_i,
            extras.range_min.unwrap_or(0),
            extras.range_max.unwrap_or(i32::MAX),
        ),
        _ => false,
    }
}

fn read_plural_case_extras(
    bytecode: &[u8],
    pos: &mut usize,
    case_type: u8,
) -> Result<PluralCaseExtras, core::fmt::Error> {
    match case_type {
        0x01 => {
        ...
        0x07 => {
            ...
            let min = i32::from_be_bytes(bytecode[*pos..*pos + 4].try_into().unwrap());
            *pos += 4;
            let max = i32::from_be_bytes(bytecode[*pos..*pos + 4].try_into().unwrap());
            *pos += 4;
            Ok(PluralCaseExtras {
                exact_val: None,
                range_min: Some(min),
...
fn read_len_string<'a>(bytecode: &'a [u8], pos: &mut usize) -> Result<&'a str, core::fmt::Error> {
    ...
    *pos += 4;
    ...
    let s = core::str::from_utf8(&bytecode[*pos..*pos + len]).map_err(|_| core::fmt::Error)?;
    *pos += len;
    Ok(s)
}
...
fn format_mf2_match<W: core::fmt::Write>(
    bytecode: &[u8],
    pos: &mut usize,
    locale: &str,
    params: &[(&str, &str)],
    ...
    *pos += 2;
    ...
    let mut selector_values: alloc::vec::Vec<SelectorState> =
        alloc::vec::Vec::with_capacity(num_selectors);
    for sel in &selectors {
        let rv = resolved
            .get(sel)
            .cloned()
            .unwrap_or_else(|| SelectorState::from_test(TestFunctionValue::bad_operand_fallback()));
        selector_values.push(rv);
    }

    if *pos + 2 > bytecode.len() {
        return Err(core::fmt::Error);
    }
    let num_variants = u16::from_be_bytes(bytecode[*pos..*pos + 2].try_into().unwrap()) as usize;
    *pos += 2;
    ...
    for _ in 0..num_variants {
        ...
        let num_keys = bytecode[*pos] as usize;
        *pos += 1;
        ...
        *pos += 4;
        ...
        let pat_pos = *pos;
        *pos += pat_len;
        variants.push((keys, pat_pos, pat_len));
    }
    let _ = variants_start;
...
fn format_message_depth<W: core::fmt::Write>(
    ...
    if !bytecode.is_empty() && (bytecode[0] == 0x00 || bytecode[0] > 0x0E) {
        ...
        return write_localized(locale, writer, text);
    }
    ...
    let tz = param_value(params, param_index.as_ref(), TIMEZONE_PARAM);
    let mut pos = 0;
    while pos < bytecode.len() {
        ...
        match opcode {
            0x01 => {
                ...
                pos += len;
                write_localized(locale, writer, text)?;
            }
            0x02 => {
                // Variable
                if pos + 4 > bytecode.len() {
                ...
                if let Some(val) = param_value(params, param_index.as_ref(), var_name) {
                    write_localized(locale, writer, val)?;
                } else {
                    writer.write_str("{")?;
            ...
            0x0B => {
                ...
                if let Some(val) = param_value(params, param_index.as_ref(), var_name) {
                    if flags & 0x01 == 0 {
                        write_localized(locale, writer, &html_escape(val))?;
                    } else {
                        write_localized(locale, writer, val)?;
                    }
                } else {
            ...
            0x0A => {
                ...
                let param_val = param_value(params, param_index.as_ref(), var_name).unwrap_or("0");
                ...
                for _ in 0..num_cases {
                    ...
                    pos += 1;
                    let extras = read_plural_case_extras(bytecode, &mut pos, case_type)?;
                    if pos + 4 > bytecode.len() {
                        return Err(core::fmt::Error);
                    }
                    let pat_len =
                    ...
                    let matches =
                        plural_case_matches(case_type, &extras, parsed_i as f64, parsed_i, cat);
            ...
            0x03 => {
                ...
                let count_i = float_math::trunc(parsed_val) as i64;
                ...
                for _ in 0..num_cases {
                    ...
                    let matches = plural_case_matches(case_type, &extras, parsed_val, count_i, cat);
            ...
            0x04 => {
                ...
                // Lookup parameter value
                let param_val = param_value(params, param_index.as_ref(), var_name).unwrap_or("");
            ...
            0x06 => {
                ...
                let formatted = format_date_tz(raw_val, locale, style, tz);
                write_localized(locale, writer, &formatted)?;
            }
            0x07 => {
            ...
            0x0C => {
                ...
                if flags & 0x01 == 0 {
                    ...
                    write_localized(locale, writer, raw_val)?;
                }
            }
            0x0D => {
                // Custom formatter
                // Layout: var_len + var + lit_len + literal + fmt_len + fmt + opt_len + options
                if pos + 4 > bytecode.len() {
                    return Err(core::fmt::Error);
                }
                let var_len =
                ...
                let lit_len =
                    u32::from_be_bytes(bytecode[pos..pos + 4].try_into().unwrap()) as usize;
                pos += 4;
                if pos + lit_len > bytecode.len() {
                    return Err(core::fmt::Error);
                }
                let literal = core::str::from_utf8(&bytecode[pos..pos + lit_len])
                    .map_err(|_| core::fmt::Error)?;
                pos += lit_len;
                if pos + 4 > bytecode.len() {
                    return Err(core::fmt::Error);
                ...
                if matches!(fmt_name, "test:function" | "test:select" | "test:format") {
                    let opts = parse_options(options_str);
                    let rv = if !var_name.is_empty() {
                        resolve_test_function_from_operand(
                            raw_val,
                            fmt_name,
                            &opts,
                            &BTreeMap::new(),
                    ...
                    if let Some(formatted) = format_test_function(&rv) {
                        write_localized(locale, writer, &formatted)?;
                    } else {
                        write_localized(locale, writer, raw_val)?;
                    }
...
mod formatter_unit_tests {
    ...
    #[test]
    fn rtl_text_wraps_latin_digits() {
        let bc = make_text("عدد 42");
        let mut out = String::new();
        format_message(&bc, "ar", &[], &mut out).unwrap();
        assert!(out.contains('\u{2068}'));
        assert!(out.contains('\u{2069}'));
    }

    #[test]
    fn raw_text_fast_path_applies_bidi() {
        let mut out = String::new();
        format_message(b"ID 99", "ar", &[], &mut out).unwrap();
        assert!(out.contains('\u{2068}'));
    }

    #[test]
    fn date_opcode_respects_tz_param() {
        let mut bc = Vec::new();
        bc.push(0x06);
        ...
        format_message(
            &bc,
            "en",
            &[("date", "2024-12-25T15:00:00Z"), ("tz", "America/Bogota")],
            &mut out,
        )
        .unwrap();
    ...
    fn test_plural_range_case() {
        ...
        bc.extend_from_slice(&2u16.to_be_bytes());
        bc.push(0x07);
        bc.extend_from_slice(&4i32.to_be_bytes());
        bc.extend_from_slice(&500i32.to_be_bytes());
        let many_pat = make_text("many");
        bc.extend_from_slice(&(many_pat.len() as u32).to_be_bytes());
        bc.extend_from_slice(&many_pat);
        bc.push(0x00);
        ...
        let mut out = String::new();
        format_message(&bc, "en", &[("count", "150")], &mut out).unwrap();
        assert_eq!(out, "many");

        let mut out = String::new();
        format_message(&bc, "en", &[("count", "500")], &mut out).unwrap();
        assert_eq!(out, "many");

...
fn parse_iso(s: &str) -> Option<DateParts> {
    ...
    Some(DateParts { year, month, day, hour, minute })
}
...
fn unix_to_parts(secs: i64) -> DateParts {
    ...
    DateParts { year, month: m, day: d, hour, minute }
}
...
pub fn format_date(value: &str, locale: &str, style: DateStyle) -> String {
    ...
    let date_str = match lang.to_lowercase().as_str() {
        ...
        "de" | "at" | "cs" | "sk" | "pl" | "hu" | "ru" | "uk" | "be" | "bg"
        | "sr" | "hr" | "bs" | "mk" | "ro" | "sl" | "lv" | "lt" | "et"
        | "el" | "tr" | "ka" | "hy" | "az" | "kk" | "ky" | "uz" | "tk" | "mn" => {
            alloc::format!("{:02}.{:02}.{}", parts.day, parts.month, parts.year)
        }
        "fr" | "es" | "pt" | "it" | "nl" | "da" | "sv" | "nb" | "fi" | "af"
        | "ca" | "gl" | "eu" | "ar" | "fa" | "ur" | "hi" | "bn" | "pa"
        | "gu" | "mr" | "ta" | "te" | "kn" | "ml" | "si" | "my" | "km"
        | "lo" | "th" | "vi" | "id" | "ms" | "sw" => {
            alloc::format!("{:02}/{:02}/{}", parts.day, parts.month, parts.year)
        }
...
mod tests {
    ...
    #[test]
    fn iso_date_english() {
        assert_eq!(format_date("2024-12-25", "en", DateStyle::Date), "12/25/2024");
    }

    #[test]
    fn iso_date_german() {
        assert_eq!(format_date("2024-12-25", "de", DateStyle::Date), "25.12.2024");
    }

    #[test]
    fn iso_date_japanese() {
        assert_eq!(format_date("2024-12-25", "ja", DateStyle::Date), "2024年12月25日");
    }

    #[test]
    fn iso_date_french() {
        assert_eq!(format_date("2024-12-25", "fr", DateStyle::Date), "25/12/2024");
    }

    #[test]
    fn time_style() {
        assert_eq!(format_date("2024-12-25T14:30:00", "en", DateStyle::Time), "14:30");
    }

    #[test]
    fn datetime_style_english() {
        assert_eq!(format_date("2024-12-25T14:30:00", "en", DateStyle::DateTime), "12/25/2024, 14:30");
    }

    #[test]
    fn unix_timestamp_integer() {
        assert_eq!(format_date("1735138200", "en", DateStyle::Date), "12/25/2024");
    }

    #[test]
    fn invalid_value_returns_raw_string() {
        assert_eq!(format_date("not-a-date", "en", DateStyle::Date), "not-a-date");
    }

    #[test]
    fn german_locale_format() {
        assert_eq!(format_date("2024-03-15", "de", DateStyle::Date), "15.03.2024");
    }

    #[test]
    fn fallback_locale_format() {
        assert_eq!(format_date("2024-03-15", "xx", DateStyle::Date), "2024-03-15");
    }

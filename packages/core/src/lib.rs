...
/// Bidirectional text isolates for RTL locales.
pub mod bidi;
...
pub mod store;
/// Lock-free RCU cell for one `TranslationStore` instance.
pub mod store_cell;
/// Scoped store handle registry (`std` only).
#[cfg(feature = "std")]
...
pub use store::{
    init_embedded, load_static_bytes, locale_parent, translate_to_writer_with_status, StoreData,
    TranslateStatus,
};
#[cfg(feature = "std")]
pub use store_registry::{create_store, destroy_store, StoreHandle};
#[cfg(test)]

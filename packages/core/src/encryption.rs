...
#[cfg(feature = "encryption")]
pub fn encrypt_aes_gcm(plaintext: &[u8]) -> Result<Vec<u8>, &'static str> {
    if !KEY_SET.load(Ordering::SeqCst) {
        return Err("Decrypt key not configured");
    }
    ...
    #[cfg(not(feature = "std"))]
    {
        ...
        Err("Encryption requires std")
    }
}

/// Decrypts AES-256-GCM bytes (expects prepended 12-byte nonce).
#[cfg(feature = "encryption")]
pub fn decrypt_aes_gcm(data: &[u8]) -> Result<Vec<u8>, &'static str> {
    if data.len() < 12 {
...
pub fn encrypt_aes_gcm(_plaintext: &[u8]) -> Result<Vec<u8>, &'static str> {
    Err("Encryption support not enabled")
}

#[cfg(not(feature = "encryption"))]
pub fn decrypt_aes_gcm(_data: &[u8]) -> Result<Vec<u8>, &'static str> {
    Err("Encryption support not enabled")
}

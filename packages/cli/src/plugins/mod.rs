...
pub const KNOWN_PLUGINS: &[(&str, &str)] = &[("crowdin", "cargo install l10n4x-plugin-crowdin")];
...
fn run_in_process_plugin(
    ...
    match plugin_id {
        ...
        "crowdin" => {
            l10n4x_plugin_crowdin::run_sync(ctx, direction, out.map(Path::new), from.map(Path::new))
        }

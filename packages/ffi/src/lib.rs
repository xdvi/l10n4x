...
use l10n4x_core::store::{
    ...
    set_fallback_locale, translate_to_writer_with_status,
    translate_to_writer_with_status_for_store, TranslateStatus,
};
...
fn clear_translate_cache_for_store(store_id: u32) {
    LAST_TRANSLATE.with(|cell| {
        let mut slot = cell.borrow_mut();
        if slot
            .as_ref()
            .is_some_and(|entry| entry.store_id == store_id)
        {
            *slot = None;
        }
    });
...
fn parse_store_handle(raw: u32) -> Result<StoreHandle, i32> {
    ...
    StoreHandle::from_raw(raw).map_err(core_error_to_ffi)
}
...
fn resolve_translation(
    ...
    let params_key = if params.is_empty() {
        if let Some(cached) = cache_lookup(store_id, locale_hash, key_hash, context_hash, 0) {
            return cached;
        }
        0
    } else {
        let pk = hash_params(params);
        if let Some(cached) = cache_lookup(store_id, locale_hash, key_hash, context_hash, pk) {
            return cached;
        }
        pk
    };
    ...
    let status = if store_id == 0 {
        translate_to_writer_with_status(locale, key_hash, context_hash, params, &mut resolved)
    } else {
        translate_to_writer_with_status_for_store(
            handle,
            locale,
            key_hash,
            context_hash,
...
fn resolve_with_param_pairs(
    ...
    if pairs.len() <= MAX_STACK_PARAMS {
        ...
        let refs: Vec<(&str, &str)> = pairs
            ...
            .collect();
        resolve_translation(store_id, handle, locale, key_hash, context_hash, &refs)
    }
}
...
#[unsafe(no_mangle)]
pub extern "C" fn l10n4c_store_translate(
    store_handle: u32,
    locale: *const c_char,
    key_hash: u64,
    buf: *mut u8,
...
#[unsafe(no_mangle)]
pub extern "C" fn l10n4c_store_translate_with_params(
    store_handle: u32,
    locale: *const c_char,
    key_hash: u64,
    params: *const L10n4cParam,

...
use l10n4c::{
    l10n4c_clear, l10n4c_compile, l10n4c_load_locale, l10n4c_load_pak_directory,
    l10n4c_set_encryption_key, l10n4c_set_fallback_locale, l10n4c_translate,
    l10n4c_translate_with_params,
};
...
fn translate_with_params_helper(
    ...
    let mut buf = [0u8; 512];
    let written =
        l10n4c_translate_with_params(locale, key, params_json, buf.as_mut_ptr(), buf.len());
    std::str::from_utf8(&buf[..written]).unwrap().to_string()
}
...
fn test_dynamic_translation_flow() {
    ...
    let load_es_res = l10n4c_load_locale(
        locale_es.as_ptr(),
        c_es_json.as_ptr(),
        prefix_errors.as_ptr(),
    );
    assert!(load_es_res);
    ...
    assert!(l10n4c_load_locale(
        locale_en.as_ptr(),
        c_en_fallback.as_ptr(),
        std::ptr::null()
    ));
...
fn test_compiler_and_pak_loading() {
    ...
    assert!(l10n4c_set_encryption_key(
        custom_key.as_ptr(),
        custom_key.len()
    ));
...
fn test_variable_interpolation_and_plurals() {
    ...
    let str_welcome = translate_with_params_helper(
        locale_en.as_ptr(),
        key_welcome.as_ptr(),
        params_welcome.as_ptr(),
    );
    assert_eq!(str_welcome, "Hello Diego!");

    // Test plural zero
    let params_zero = CString::new(r#"{"count": "0"}"#).unwrap();
    let str_zero = translate_with_params_helper(
        locale_en.as_ptr(),
        key_messages.as_ptr(),
        params_zero.as_ptr(),
    );
    ...
    let str_one = translate_with_params_helper(
        locale_en.as_ptr(),
        key_messages.as_ptr(),
        params_one.as_ptr(),
    );
    ...
    let str_other = translate_with_params_helper(
        locale_en.as_ptr(),
        key_messages.as_ptr(),
        params_other.as_ptr(),
    );

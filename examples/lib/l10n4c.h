...
/* ── Scoped stores (multi-tenant; handle 0 is reserved / invalid) ─────────── */

/**
 * Creates an isolated translation store. Returns non-zero handle, or 0 on failure.
 */
uint32_t l10n4c_store_create(void);
...
int32_t l10n4c_store_translate(uint32_t store_handle, const char *locale, uint64_t key_hash,
                               uint8_t *buf, size_t max_len);

/** Translates a key with typed parameters from a scoped store. */
int32_t l10n4c_store_translate_with_params(uint32_t store_handle, const char *locale,
                                           uint64_t key_hash, const L10n4cParam *params,
                                           size_t param_count, uint8_t *buf, size_t max_len);

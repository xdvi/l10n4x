...
// fnv1a_64 computes the FNV-1a 64‑bit hash used by the l10n4c key-hash API.
func fnv1aKey(key string) uint64 {
	hash := fnv1aOffset
...
func (t *Translator) Translate(locale, key string) string {
	...
	keyHash := C.uint64_t(fnv1aKey(key))

	ptr := C.l10n4c_translate_alloc(cLocale, keyHash)
	if ptr == nil {
		return key
	}

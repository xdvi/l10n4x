...
1. Every push to `main` is scanned for conventional commits since the last release.
2. When `feat:` or `fix:` commits accumulate, release-please opens a release PR.
3. Merging the release PR creates a git tag, GitHub release, and publishes to crates.io.
4. Changelog entries are generated automatically from commit messages.

### Release PR format

```
release: l10n4x 0.2.0
```

The PR body contains the auto-generated changelog. Review it, adjust if needed, and merge.

### Manual steps (if automation breaks)

1. Check `.release-please-manifest.json` reflects the current version.
2. Verify `CHANGELOG.md` is up to date.
3. The tag and crates.io publish are handled by CI on PR merge.
